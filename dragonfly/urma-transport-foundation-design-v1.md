# Dragonfly URMA Transport Foundation Design v1
...
>
> [架构设计] 本文不修改 Dragonfly/UMDK 源码，不设计 Scheduler endpoint 下发，不把 perftest 的 benchmark management protocol 直接作为生产协议，也不在第一阶段启用 one-sided READ/WRITE 或远端 Segment 访问。
...
8. [架构设计] OOB 使用静态配置的 TCP 地址，只交换 EID、opaque Jetty descriptor、opaque Segment descriptor 与 capability。token/authorization material 由双端静态配置提供，不经明文 OOB 发送。
9. [架构设计] Segment descriptor 虽在 v1 handshake 中交换并校验，但 RC SEND/RECV 不需要 `urma_import_seg()`；v1 不导入远端 Segment，避免无意扩大远端内存访问权限。
...
| Segment | `urma_register_seg`, `urma_get_seg_ctx`, `urma_import_seg`, 对应反操作 | [源码确认] `urma_api.h:809–857`。 | [架构设计] 注册本地 RX/TX region，导出 opaque descriptor；v1 不 import remote Segment。 |
...
- [架构设计] allowlist 至少覆盖 `urma_(init|uninit|get_device_by_name|get_eid_list|free_eid_list|query_device|create_context|delete_context|create_jfc|delete_jfc|create_jetty|modify_jetty|delete_jetty|get_rjetty|put_rjetty|import_jetty|unimport_jetty|bind_jetty|unbind_jetty|register_seg|unregister_seg|get_seg_ctx|put_seg_ctx|post_jetty_send_wr|post_jetty_recv_wr|poll_jfc)`。
...
- [架构设计] 对 `urma_context_t`、`urma_jfc_t`、`urma_jetty_t`、`urma_target_jetty_t`、`urma_target_seg_t` 在 Rust 层按 opaque handle 使用；需要 EID、Jetty ID、descriptor 时通过 shim 或 `urma_get_rjetty()`/`urma_get_seg_ctx()` 复制出来，不读取 private fields。
...
| descriptor extraction | [架构设计] local Jetty/Segment 仍有效。 | [架构设计] 复制为 owned `Vec<u8>`，立即调用 `urma_put_rjetty()`/`urma_put_seg_ctx()`。 |
...
- [架构设计] OOB TCP codec/timeout 可运行在 Tokio；涉及 `urma_get_rjetty/get_seg_ctx/import/bind` 的步骤通过 request/reply command 在 I/O thread 执行。
...
sequenceDiagram
    ...
    RT->>U: urma_get_rjetty(local_jetty)
    RT->>U: urma_get_seg_ctx(local_segment)
    RT-->>O: EID + Jetty descriptor + Segment descriptor + capability
    O-->>RT: peer descriptors + capability
...
| remote Segment | `urma_import_seg` | [架构设计] v1 不创建。 | [架构设计] 未来若创建，先停止 one-sided WR/drain，再 `urma_unimport_seg`。 |

- [源码确认] perftest `create_duplex_ctx()` 的实际顺序为 device/context -> duplex Jetty/JFC -> registered memory -> connection info exchange -> remote Segment import -> remote Jetty connect；这证明 descriptor exchange/import 是显式步骤，不由 `urma_create_jetty()` 自动完成。
- [架构设计] foundation v1 沿用其资源依赖，但针对 SEND/RECV 删除 remote Segment import；local Segment 仍是 SEND/RECV SGE 必需资源。
...
| `OOB_EXCHANGE` | [源码确认] Segment descriptor 由 `urma_get_seg_ctx()` 取得、用完由 `urma_put_seg_ctx()` 释放。 | [架构设计] 收到 peer Hello，magic/version/role/EID/capability/descriptor lengths 全部合法，协商为 RC + SEND/RECV。 | [架构设计] 关闭 OOB socket，delete local Jetty；不调用 import/bind。 |
...
sequenceDiagram
    ...
    C->>P: ClientHello(version, nonce, capability, EID)
    P->>C: ServerHello(version, nonce, capability, EID)
    C->>P: Descriptor(Jetty blob, Segment blob)
    P->>C: Descriptor(Jetty blob, Segment blob)
    Note over C,P: each side imports remote Jetty and binds local RC Jetty
    Note over C,P: each side preposts configured RX slots
    C->>P: Ready(generation, rx_credit)
    P->>C: ReadyAck(generation, rx_credit)
    Note over C,P: Piece protocol may now use URMA SEND/RECV
```

- [架构设计] Parent/Child 均创建 duplex Jetty，因此双方都交换同类 descriptor；role 只决定谁 listen/connect 和谁先发 Hello，不改变 URMA data direction。
- [架构设计] OOB TCP frame 使用显式 magic、version、message type、payload length 和 handshake nonce；整数固定 network byte order，所有 vector/string 都有硬上限。
- [架构设计] 不直接使用 Rust `bincode`、C struct dump 或 `sizeof(urma_rjetty_t)` 作为 wire format。Jetty/Segment descriptor 是由 UMDK API 返回的 opaque bytes，外层协议只承载 length + bytes。

### 6.3 交换字段

| 字段 | 表示 | 校验与用途 |
|---|---|---|
| `EID` | [源码确认] `urma_eid_t` 是 16-byte network-order raw 值；Context 公开 local EID。 | [架构设计] 固定 16 bytes，与 Jetty descriptor 中 identity 交叉校验，并可与静态 allowlist 比较。 |
| `Jetty descriptor` | [源码确认] 使用 `urma_get_rjetty(local_jetty, &ptr, &len)` 获取，使用后 `urma_put_rjetty(ptr)`。 | [架构设计] 作为 opaque bounded blob 传输；接收端仅在 capability/length 通过后交给 `urma_import_jetty`。 |
| `Segment descriptor` | [源码确认] 使用 `urma_get_seg_ctx(local_tseg, &ptr, &len)` 获取，使用后 `urma_put_seg_ctx(ptr)`。 | [架构设计] v1 交换、记录 hash 并验证存在，但不调用 `urma_import_seg()`；为未来 one-sided/zero-copy capability negotiation 保留。 |
| `capability` | [架构设计] protocol version、transport type/mode、SEND_RECV、slot size、rx/tx depth、max SGE、max descriptor length、feature bits。 | [架构设计] 取双方交集；v1 只接受 RC + SEND_RECV + protocol v1，拒绝隐式降级。 |
...
struct TransportCapabilities {
    ...
    features: FeatureBits, // SEND_RECV required; REMOTE_SEGMENT disabled in v1
    slot_size: u32,
    rx_depth: u32,
    tx_depth: u32,
    max_sge: u16,
...
- [架构设计] `slot_size` 协商取双方可接收上限的较小值；v1 配置若与本地 pre-registered slot layout 不兼容则握手失败，不在握手中动态重分配 registered memory。
- [架构设计] Segment descriptor 不包含静态 auth token；`urma_import_jetty()` 所需 token value来自本地静态配置。日志只记录 descriptor length/hash，不记录原始 bytes。
- [待验证] `urma_get_rjetty()`/`urma_get_seg_ctx()` 返回 descriptor 是否包含 provider-specific extension、是否可跨不同 patch version/provider 互通，以及 import API 对 buffer alignment/ownership 的要求。
- [待验证] 当前 UMDK perftest 同时存在直接交换固定 `urma_*` struct 与 `get_*_ctx` variable blob 的路径；Dragonfly 应以目标 provider 实测确定唯一 descriptor 形式，不混用两种格式。
...
| malformed/oversized descriptor | [架构设计] 立即关闭，记录 peer + length，不把 bytes 交给 FFI。 | [架构设计] protocol error/EOF。 |
| import/bind failure | [架构设计] 若已 import 则 unimport；删除 local Jetty。 | [架构设计] OOB BindFailed 后关闭。 |
| RX prepost partial failure | [架构设计] Jetty -> ERROR，drain 已 post WR，unbind/unimport/delete。 | [架构设计] 不发送 Ready，或发送 ReadyFailed。 |
...
- [架构设计] `UrmaEndpoint` 是静态配置与 peer identity，不包含 raw Jetty/Segment descriptor；descriptor 只在一次 OOB session 的临时 `PeerHello` 中存在。
...
| `UrmaBufferPool` | [架构设计] local Segment 属于 Runtime；v1 exchange remote descriptor 但不 import，不改变 copy-mode ownership。 |
...
8. [待验证] v1 不调用 `urma_import_seg()`；Segment descriptor exchange 不授予远端 READ/WRITE 权限，运行 trace 可证明只有 SEND/RECV opcode。
...
4. [待验证] `urma_get_rjetty()` 与 `urma_get_seg_ctx()` descriptor 的跨进程、跨小版本、跨 provider兼容性和 maximum length。
5. [待验证] `urma_import_jetty()` 所需 token policy/value 如何安全 provision；v1 静态 secret 的配置文件权限、轮换和日志脱敏规则。
6. [待验证] `urma_bind_jetty()` 双端并发调用、`URMA_EEXIST`、native state query 和 retry 的精确语义。
...
9. [待验证] Segment registration 的 page/alignment/cacheability/access flag 与普通 allocator/huge page/UBS Memory 的约束；v1 先使用最保守的 local SEND/RECV access。
