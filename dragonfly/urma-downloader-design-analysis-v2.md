...
9. [架构推断] 协议采用 URMA framing + Dragonfly semantic payload，不直接把面向 byte stream 的 Vortex framing 套在 message completion 上；未来再演进为 Vortex codec + URMA transport adapter。
10. [架构推断] 第一阶段固定 copy mode：`CQE -> RX Buffer -> memcpy -> Bytes -> PieceContentStream`，以解耦 registered buffer 与 Storage 生命周期；接口保留 `ReceivedData`/`BufferLease` 以支持后续 zero-copy。
11. [架构推断] 实施顺序调整为 standalone 闭环、transport layer、Parent Server、Child Downloader、性能优化五个 Phase，功能正确性先于 multi request、zero-copy 和 UBS Memory。
...
## 4. Protocol Design Decision

### 4.1 现有语义与传输模型差异

- [源码确认] Dragonfly 当前 TCP/QUIC client 与 Parent server 都使用 Vortex 表达 standard Piece 请求、`PieceContent` metadata 和业务错误；header 与 metadata 读完后，后续连接/QUIC stream 中的 bytes 被直接作为 Piece body 消费。
- [URMA源码确认] URMA SEND/RECV 不是 byte stream：发送方提交一条 WR/message，接收方预投一个 receive buffer，并由一条 recv CQE 给出该 message 的 `completion_len`。CQE 边界是 message completion 边界，不能假定为可连续 `read_exact()` 的 socket 字节流。
- [架构推断] 因此第一阶段采用 **URMA framing + Dragonfly semantic payload**：URMA envelope 负责 message 边界、版本、`request_id`、`sequence` 和长度；payload 保留 Dragonfly Piece 的业务含义，不改变 `task_id + piece_number`、offset、digest、有序 body 和错误语义。

### 4.2 Message 与 Piece 下载语义映射

| URMA message | 方向 | 对应现有 Piece 下载语义 |
|---|---|---|
| `Request` | Child -> Parent | [架构推断] 对应 Vortex `DownloadPiece` 请求，携带 `task_id + piece_number`，触发 Parent 查找并上传 standard Piece。 |
| `Metadata` | Parent -> Child | [架构推断] 对应 Vortex `PieceContent` 的控制信息，提供 `offset + piece_length + digest`；收到它后 `UrmaDownloader` 即可返回 stream，而不等待 body 完成。 |
| `Data` | Parent -> Child | [架构推断] 对应 Vortex metadata 后的 Piece body byte range；多条 Data 按 `sequence` 拼成一个有序 `PieceContentStream`。message 边界不是 Storage 业务边界。 |
| `End` | Parent -> Child | [架构推断] 显式表达 URMA message 序列结束并校验累计长度/chunk 数；它只产生合法 EOF，不代表 Storage 已完成 digest 校验或 metadata commit。 |
| `Error` | Parent -> Child | [架构推断] 对应 Vortex Error/Parent 业务失败；在 Metadata 前结束 downloader future，在 streaming 阶段产生 stream error，不得伪装为 EOF。 |

### 4.3 第一阶段不直接复用 Vortex stream framing

- [架构推断] Vortex 当前 framing 建立在“先从 byte stream 读取固定 header，再按 header length 读取 payload，最后把剩余 stream 当作 Piece body”的读取模型上；URMA 的接收入口则是一系列预投 buffer 的独立 completions，没有天然的连续 reader cursor。
- [架构推断] 若直接把 TCP Vortex stream decoder 套在 URMA 上，还需要额外实现跨 CQE 拼接、半 header/半 payload 缓存和 body EOF 推断，既掩盖 URMA 原生 message 边界，也使 RX slot 的回收与虚拟 byte stream 缓存耦合。
- [架构推断] 第一阶段的目标是先验证 URMA transport、CQE 路由和 Piece 闭环，不同时改造 Vortex codec；独立且最小的 URMA framing 能把 transport 风险与现有业务语义分开验证。
- [待实验验证] URMA envelope 的 header 开销、最大 payload、message 数和 codec CPU 成本需在真实 Piece 尺寸上测量，再决定是否值得统一 codec。

### 4.4 后续演进方向

- [架构推断] 稳定后可把 Vortex 中与 transport 无关的 Request/Metadata/Error 编解码提取为 **Vortex codec**，再增加 **URMA transport adapter**，负责将 codec frame 映射到一条或多条 URMA message completion。
- [架构推断] 演进目标是复用 Dragonfly semantic codec，而不是强迫 URMA 模拟 TCP socket；adapter 仍负责 `request_id` demux、Data sequence、End、buffer lease 和 CQE error 映射。
- [待实验验证] 是否扩展 Vortex 本身以原生表达 Data/End，或在 adapter 内保留轻量 envelope，需要结合兼容性、跨语言实现和 multi-request 需求另行决策。
...
- [URMA源码确认] recv CQE 只说明对应 RX buffer 已完成设备写入并给出有效长度；在重新 post receive WR 前，CPU 必须完成对该有效范围的读取。
- [架构推断] `CompletionPoller` 校验 CQE、slot generation 和 message header 后，把 Data payload `memcpy` 到独立 `Bytes`；`Bytes` 进入 bounded channel/`PieceContentStream`，RX slot 则可在投递策略允许时 repost。
- [源码确认] Storage 会把 `Bytes` 移入 blocking `pwritev` 路径，future 超时或取消时，已启动的 blocking write 仍可能继续持有它。
- [架构推断] copy mode 的关键收益不是减少复制，而是让 **URMA buffer 生命周期与 Storage 生命周期解耦**：registered RX slot 的回收、repost 和 connection drain 不必等待 Storage 写盘、CRC32 或最后一个 `Bytes` clone/drop。

### 7.2 后续：zero-copy lease

```text
CQE -> RecvLease -> Bytes owner -> Storage
```

- [架构推断] zero-copy 版本由 recv CQE 创建 `RecvLease`；由该 lease 支撑的 `Bytes` owner 持有 registered slot，Storage 消费完且最后一个 owner drop 后，才把 slot 归还给 runtime/poller 并 repost。
- [架构推断] lease 必须携带 slot id、slot generation、connection generation 和有效范围；Drop 只能发出归还通知，实际 repost 仍由拥有 URMA handle 的 I/O 线程执行。
- [架构推断] zero-copy 会把可用 receive credit 直接绑定到 Storage 消费速度，因此需要在 slot 数、channel backpressure、blocking write 取消和 shutdown drain 之间建立更严格的不变式。
- [待实验验证] 从 registered memory 构造自定义 owner `Bytes` 的安全封装、DMA/cache 一致性、跨线程 Drop 和 unregister fencing 必须在目标 provider 上单独验证。

### 7.3 保留所有权抽象

- [架构推断] 不应让 `CompletionPoller` 的稳定接口固定返回 `Bytes`。建议内部先返回 `ReceivedData`，其 payload 可以是 copy-owned buffer，也可以是 `BufferLease`/`RecvLease`，由 delivery/stream adapter 再转换为当前 `PieceContentStream` 所需的 `Bytes`。
- [架构推断] 第一阶段的 `ReceivedData::Copied(Bytes)` 仍走 copy mode；未来增加 `ReceivedData::Leased(RecvLease)` 时，不需要重写 CQE 路由、RequestState 和协议状态机。
- [架构推断] `BufferLease` 是所有权与归还协议，不应泄漏 raw pointer、Segment handle 或 repost 权限给 Storage。

## 8. 并发模型：Task、Piece、Chunk、Jetty、BufferSlot

### 8.1 Dragonfly 现有并发语义
...
- [架构推断] 在 Dragonfly 主链之外先完成最小 Parent/Child demo，建立 duplex Jetty、预投 RX、执行 OOB Ready barrier，并跑通 `Request -> Metadata -> Data -> End` 闭环；同时用 `Error` 覆盖非法请求或主动注入失败。
- [架构推断] Request/Metadata/Data/End 必须全部走 URMA SEND/RECV，OOB TCP 只做 descriptor/能力交换，不承载业务 payload。
- [待实验验证] 验收 CQE status、`completion_len`、`user_ctx`、多 Data message、RX repost、TX 回收以及正常/异常 shutdown drain；此阶段不依赖 Storage 或 `Downloader` trait。

### Phase 1：URMA transport layer

- [架构推断] 实现 `runtime`：init、device/context、JFC、I/O thread、Ready 与严格逆序 shutdown。
- [架构推断] 实现 `connection`：OOB descriptor 交换、Jetty import/bind、generation、request registry 和发送命令。
- [架构推断] 实现 `buffer`：registered Segment、RX/TX slots、lease/state machine、`user_ctx` 索引和 receive repost。
- [架构推断] 实现 `completion`：CQ polling、status/length 校验、send 回收、recv 路由、backpressure 与 drain；接口保留 `ReceivedData`/`BufferLease` 抽象，第一阶段使用 copied variant。
- [架构推断] 实现 `protocol`：URMA envelope 与 Request/Metadata/Data/End/Error codec、sequence/length 状态机和纯内存单元测试。
- [待实验验证] Phase 1 完成门槛是 feature-on 真机可反复 start/connect/transfer/stop 且无 slot/handle 泄漏，feature-off 与现有 TCP/QUIC build 行为不变。

### Phase 2：Parent `UrmaServer`

- [架构推断] 接入现有 `Storage::upload_piece()`，从 `task_id + piece_number` 获得 Piece metadata 与 `RangeReader`。
- [架构推断] 先发送 Metadata，再把 RangeReader 分块读入 TX slots 并发送多条 Data，最后发送 End；not-found、Storage read 失败或资源不足发送 Error。
- [待实验验证] 使用大于 slot payload 的 Piece 验证 offset、length、digest、上传限速边界、多 chunk 发送和 Parent 中途失败清理。

### Phase 3：Child `UrmaDownloader`

- [架构推断] 实现现有 `Downloader` trait 的 standard `download_piece()`：发送 Request、等待 Metadata，把 Data 的 copy-owned `Bytes` 暴露为 `PieceContentStream`，并把 End/Error/CQE failure 映射为 EOF 或 error。
- [架构推断] 接入现有 Storage 写入、CRC32/digest 和 metadata commit；persistent 与 persistent-cache 方法仍明确 unsupported，TCP/QUIC 路径保持不变。
- [待实验验证] 验收完整落盘、连续重复下载、metadata/body timeout、stream early Drop、digest mismatch 和 transfer 中 shutdown。

### Phase 4：性能优化

- [架构推断] `multi request`：移除单 request permit，启用 shared Jetty 上的 request_id demux、公平调度、显式 credit 或经验证的 RNR backpressure，并评估多 Jetty 分片。
- [架构推断] `zero copy`：沿 `CQE -> RecvLease -> Bytes owner -> Storage` 演进，让最后一个 owner Drop 驱动 slot 归还，同时保持 `ReceivedData`/`BufferLease` 边界。
- [架构推断] `UBS Memory`：评估设备/平台提供的 UBS Memory 能力、注册方式和对 RX/TX/Storage 路径的适用性，不在确认 API、ownership 和一致性约束前替换普通 registered Segment。
- [待实验验证] 优化必须以 copy mode 基线对照吞吐、尾延迟、CPU、memory bandwidth、slot/RNR 和 Storage I/O；只有端到端收益明确时才进入默认路径。
