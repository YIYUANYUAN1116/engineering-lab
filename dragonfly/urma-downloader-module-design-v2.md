...
>
> [架构设计] 第一阶段不实现 zero-copy、READ/WRITE、Scheduler endpoint 扩展和 multi-peer 连接池；本文只为 zero-copy 保留 `ReceivedData`/`BufferLease` 演进边界。
...
9. [架构设计] wire protocol 采用 URMA framing + Dragonfly semantic payload；第一阶段不直接复用 Vortex stream framing，未来可演进为 Vortex codec + URMA transport adapter。
10. [架构设计] 第一阶段 buffer path 固定为 copy mode，但 CompletionPoller 以 `ReceivedData` 为稳定输出边界，为后续 `RecvLease` zero-copy 保留所有权抽象。
11. [架构设计] 实现按 Phase 0–4 推进：standalone transport demo、transport layer、Parent `UrmaServer`、Child `UrmaDownloader`、性能优化。
...
struct CompletionPoller {
    ...
    pending_deliveries: VecDeque<PendingDelivery<ReceivedData>>,
...
- [架构设计] CompletionPoller 的稳定输出使用 `ReceivedData`，不固定为 `Bytes`。第一阶段构造 `ReceivedData::Copied(Bytes)` 后对 per-request bounded channel 调 `try_send`；若 channel full，将 `(slot_id, ReceivedData, state)` 放入 `pending_deliveries`，**暂不 repost 该 RX slot**，以 RX slot 作为 receive credit 形成有界背压。
- [架构设计] copy mode 的 pending data 已与 slot memory 解耦，但 slot 仍被故意扣留；当 `try_send` 成功或 request 进入 Draining 丢弃该 data 后，才 repost receive WR。
- [待验证] command batch、CQ poll batch、idle spin/yield/sleep 策略需根据真实 CPU 占用与延迟调整。

## 6. 最小 URMA Piece Message Protocol

### 6.1 Protocol Design Decision

- [源码确认] Dragonfly 当前 TCP/QUIC client 与 server 使用 Vortex 表达 `DownloadPiece`、`PieceContent` metadata 和 Error；读取 Vortex header/metadata 后，剩余 TCP/QUIC stream bytes 直接作为 Piece body。
- [源码确认] 现有 Storage 的稳定边界是 `PieceContentStream = BoxStream<'static, io::Result<Bytes>>`，它关心有序 bytes、offset、length 和 digest，不依赖 Vortex frame 或 socket 类型。
- [URMA源码确认] URMA SEND/RECV 是 message completion 模型，不是 byte stream：每个 receive WR 对应预投 buffer，recv CQE 以 `completion_len` 标出一条已完成 message 的有效范围。
- [架构设计] 第一阶段采用 **URMA framing + Dragonfly semantic payload**。32-byte URMA header 处理 message 边界、版本、request demux、sequence 和长度；payload 沿用 Dragonfly standard Piece 的定位、metadata、body 与错误语义。

| URMA message | Dragonfly Piece 对应关系 |
|---|---|
| `Request` | [架构设计] 对应 Vortex `DownloadPiece`：`task_id + piece_number` 定位 Parent 上的 standard Piece。 |
| `Metadata` | [架构设计] 对应 Vortex `PieceContent` 控制信息：`offset + piece_length + digest`；它唤醒 `download_piece()` 并允许返回 body stream。 |
| `Data` | [架构设计] 对应 metadata 后的 Piece body bytes；多条 message 依 sequence 适配为一个 `PieceContentStream`，其 message 边界不暴露为业务边界。 |
| `End` | [架构设计] 为 message transport 补充显式 EOF 与累计长度/chunk 数校验；只结束 stream，Piece 成功仍由 Storage 写入、digest 和 metadata commit 决定。 |
| `Error` | [架构设计] 对应 Vortex Error/Parent 业务失败；Metadata 前失败 future，Metadata 后失败 stream，不能转换成 End。 |

- [架构设计] 第一阶段不直接复用 TCP Vortex stream framing：现有 decoder 假设可从连续 reader 中先读 header、再按 length 读 payload，并把后续 EOF 解释为 body 结束；套到 URMA 会额外引入跨 CQE 拼接、半 frame 缓存和虚拟 reader cursor，并把 RX slot 回收耦合到 stream buffer。
- [架构设计] 独立 URMA framing 能优先隔离验证 CQE、slot、repost、request demux 与 End/Error；它不重新定义 Dragonfly Piece 业务模型，也不阻止 codec 复用。
- [架构设计] 后续可将 transport-neutral 业务编码提取为 **Vortex codec**，由 **URMA transport adapter** 把 codec frame 映射到 URMA completions；adapter 继续负责 request_id、Data sequence、End、buffer lease 和 CQE error。
- [待验证] 是否让未来 Vortex codec 原生包含 Data/End，还是保留 URMA envelope 扩展，需结合兼容性、multi-request、跨语言实现和性能数据决定。
...
### 6.9 OOB handshake 与 Piece protocol 的边界
...
```text
CQE -> RX Buffer -> memcpy -> Bytes -> PieceContentStream
```

- [URMA源码确认] 成功 recv CQE 把 RX slot 从 device-owned `Posted` 转为 CPU 可读的 `Completed`，`completion_len` 决定有效范围；slot 在重新 post 后又回到 device-owned 状态。
- [架构设计] poller 校验 CQE/header 后把 Data payload 复制进独立 `Bytes`。stream/channel 持有 `Bytes`，而 RX slot 可在投递/backpressure 规则允许时回到 receive queue。
- [源码确认] Storage 可能把 `Bytes` 移入 blocking `pwritev` task；即使 async future 被取消，blocking write 仍可能继续持有 buffer。
- [架构设计] 因此 copy mode 明确解耦 **buffer 生命周期** 与 **Storage 生命周期**：slot 的 repost、connection drain 和 Segment 注销不等待 Storage 写盘或 `Bytes` owner 释放。

### 7.2 `ReceivedData` / `BufferLease` 抽象
...
struct RecvLease {
    ...
    connection_generation: ConnectionGeneration,
    range: Range<usize>,
    return_tx: mpsc::UnboundedSender<ReturnRecvSlot>,
}
```

- [架构设计] `CompletionPoller` 不应固定返回 `Bytes`；它产出 `ReceivedData`，delivery/stream adapter 再把第一阶段的 `Copied(Bytes)` 交给 `UrmaRequestState`。
- [架构设计] `BufferLease`/`RecvLease` 只暴露受控的有效数据范围和归还语义，不向 Storage 暴露 raw pointer、Segment handle 或直接 repost 能力。
- [架构设计] 第一阶段只实现 `Copied` 路径，但在 command、pending delivery 和 request delivery 边界保留 enum/trait，使 zero-copy 不需要重写 completion routing 与 protocol state machine。

### 7.3 后续 zero-copy

```text
CQE -> RecvLease -> Bytes owner -> Storage
```

- [架构设计] 未来由 `RecvLease` 支撑自定义 owner `Bytes`；最后一个 owner drop 后发送 `ReturnRecvSlot`，由 poller 校验 generation 并 repost，不能从任意 Tokio/Storage 线程直接操作 URMA handle。
- [架构设计] leased slot 在 Storage 完成前持续占用 receive credit；stream Drop、blocking write 未结束和 shutdown 都必须等待 lease 归还或进入显式 drain/fence 流程。
- [待验证] 自定义 owner `Bytes` 的安全实现、DMA/cache 一致性、Drop 线程、归还队列失效处理以及 unregister fencing 需通过目标 provider 真机测试后才能启用。

## 8. CQE 到 `PieceContentStream` 的精确路径
...
## 10. 需要修改或新增的 Dragonfly 文件

### 10.1 `dragonfly-client-storage`
...
### 10.3 Rust workspace 与 lockfile
...
### Phase 0：URMA standalone transport demo

- [架构设计] 在 Dragonfly crate 接入前建立独立 Parent/Child demo，完成 OOB descriptor 交换、duplex Jetty import/bind、RX 预投和 Ready barrier。
- [架构设计] demo 必须完成 `Request -> Metadata -> Data -> End` 闭环，并以 Error 覆盖非法 Request；所有五类 message 走 URMA SEND/RECV，OOB 不承载 Piece payload。
- [架构设计] 建议拆成原 commit 1/4 的前置切片：先完成 feature/FFI build、纯 protocol codec，再完成固定 payload 与完整 message 闭环；codec 测试保留大小端序、截断、超长、未知 version/type/flag 和 UTF-8 失败。
- [待验证] 验收 `feature=urma` 链接 `liburma`、CQE status、`completion_len`、`user_ctx`、TX 回收、RX repost、多 Data message 和退出 drain；默认 feature-off 在无 UMDK 机器仍通过。

### Phase 1：URMA transport layer

- [架构设计] `runtime`：I/O thread、Context/JFC/Segment 创建、command queue、Ready、反复 start/stop 和显式 shutdown/join。
- [架构设计] `connection`：OOB、Jetty/imported target、generation、request registry、post command 与故障广播。
- [架构设计] `buffer`：对齐 region、Segment 注册、RX/TX 切分、`TxSlotLease`、`RecvLease` 预留、`user_ctx` slab/index 与 slot state machine。
- [架构设计] `completion`：send/recv CQ batch、status/length/generation 校验、TX 回收、RX delivery/repost、bounded backpressure 和 drain；对上游产出 `ReceivedData`，第一阶段仅启用 `Copied(Bytes)`。
- [架构设计] `protocol`：固化 Request/Metadata/Data/End/Error codec 与 sequence/length 状态机；用模拟 CQE/slot 测试多 Data、Error、channel full、early Drop、stale generation、double free/repost 和半途失败清理。
- [待验证] Phase 1 的硬门槛是 standalone demo 改用完整 transport modules 后仍可连续 transfer/reconnect/shutdown，且 handle、slot 和 pending delivery 无泄漏。

### Phase 2：Parent `UrmaServer`

- [架构设计] 实现 Inbound Request -> `Storage::upload_piece()`，取得 metadata 与 `RangeReader`，发送 Metadata、多条 Data 和 End；not-found、RangeReader read failure 与资源不足发送 Error。
- [架构设计] 保留现有上传限速与 Piece 等待语义；TX slot 读入 RangeReader 后 post SEND，只在 send CQE 后归还。
- [待验证] 使用大于 slot payload 的测试 Piece，确认 RangeReader 被拆成多个 Data message，并覆盖 Parent 中途失败与 connection drain。

### Phase 3：Child `UrmaDownloader`

- [架构设计] 新增 `resource/urma_downloader.rs` 并实现 standard `Downloader::download_piece()`；metadata oneshot 决定 future 返回，bounded body mpsc/`UrmaPieceStream` 形成 `PieceContentStream`。
- [架构设计] 增加 config/validation、Task/Piece optional injection、`download.protocol="urma"` 分支、Parent/Child role 启动和 graceful shutdown；persistent 与 persistent-cache 返回 Unimplemented，TCP/QUIC 默认行为保持不变。
- [架构设计] 直接把 adapter 返回 stream 交给 Storage，复用 length/digest、CRC32 和 metadata commit，不把 CQE 视为 Piece 成功。
- [待验证] 真机覆盖单 Piece 多 chunk、连续重复下载、metadata/body timeout、Error/CQE failure、stream early Drop、digest mismatch 与 transfer 中 shutdown。

### Phase 4：性能优化

- [架构设计] **multi request**：移除单 request permit，在 shared Jetty 上启用 request_id demux、公平发送、connection 级故障广播和经验证的 RNR/显式 credit；必要时按 request_id 分片多 Jetty/poller。
- [架构设计] **zero copy**：实现 `ReceivedData::Leased(RecvLease)` 与 custom-owner `Bytes`，沿 `CQE -> RecvLease -> Bytes owner -> Storage` 传递所有权，最后一个 owner Drop 后由 poller repost。
- [架构设计] **UBS Memory**：在能力探测、注册 API、访问权限、DMA/cache 一致性和 Storage 适配明确后，评估用 UBS Memory 优化 RX/TX 或 Storage 路径；不把它作为 Phase 0–3 的正确性依赖。
- [架构设计] 补齐 request ID、post/CQE、bytes、slot/lease states、channel full、RNR、drain duration、digest result 的 tracing/metrics，并以 copy mode 建立吞吐、尾延迟、CPU 与 memory bandwidth 基线。
- [待验证] multi request、zero copy 和 UBS Memory 分别独立压测与故障注入；只有端到端收益和资源回收不变式同时成立才合入默认路径。
...
- [架构设计] copy mode 中交给 Storage 的 `Bytes` 不得引用 registered RX slot；zero-copy mode 中 leased RX slot 必须等最后一个 `Bytes` owner 归还 lease 后才能 repost 或 unregister。
